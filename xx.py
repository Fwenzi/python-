...
c.execute('''CREATE TABLE stockList
      (id text primary key,
       name text,
       newPrice text,
       priceLimit text,
...
for i in range(0,len(stock_last),13):
  l = (stock_last[i],stock_last[i+1],stock_last[i+2],stock_last[i+3],stock_last[i+4],stock_last[i+5])
  p.append(l)

print(p)
c.executemany("INSERT INTO stockList VALUES (?,?,?,?,?,?)",p)
